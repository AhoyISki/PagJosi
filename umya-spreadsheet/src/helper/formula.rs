...
use crate::{
    helper::{address::*, coordinate::*, range::*},
    structs::StringValue,
};
...
pub(crate) fn parse_to_tokens<S: Into<String>>(formula: S) -> Vec<FormulaToken> {
    ...
    while index < formula_length {
        ...
        // trim white-space
        if formula.chars().nth(index).unwrap() == self::WHITESPACE && !in_string && !in_path {
            if value != "" {
                let mut obj = FormulaToken::default();
                obj.set_value(value);
                obj.set_token_type(FormulaTokenTypes::Operand);
    ...
    // when appropriate, switching infix "+" operators to noop when

    // appropriate, identifying operand and infix-operator subtypes, and
...
pub fn adjustment_insert_formula_coordinate(
    ...
    for token in token_list.into_iter() {
        ...
        {
            let (sheet_name, range) = split_address(token.get_value());
            let unquoted = sheet_name.trim_matches(['\'', '"']);
            if ignore_worksheet
                || (unquoted == "" && worksheet_name == self_worksheet_name)
                || (unquoted == worksheet_name)
            {
                let mut coordinate_list_new: Vec<String> = Vec::new();
                let coordinate_list = get_split_range(range);
                for coordinate in &coordinate_list {
